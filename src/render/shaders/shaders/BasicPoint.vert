...
void main() 
{
	...
	if (data & SELECTABLE_BIT) { // If item is selectable
		...
		if (!selected && bool(data & SUB_SELECTABLE_BIT)) { // If item is not slected but a subsection might be selected
			unsigned int pointNum = gl_VertexID;
			frag_sub_id = pointNum;
			unsigned int pointIndex = pointNum / 32;
			unsigned int pointBit = 1 << (pointNum % 32);
			selected = bool(selection_buffer[pointIndex] & pointBit);
		}
	}
