...
void ShaderManager::Init() 
{
	...
	GLCall(glBufferData(GL_UNIFORM_BUFFER, sizeof(glm::mat4) + sizeof(glm::vec3) + sizeof(uint32_t) + sizeof(uint32_t) + sizeof(uint32_t), NULL, GL_DYNAMIC_DRAW));
...
void ShaderManager::UpdateLocalUniforms(const glm::mat4& model, const glm::vec3& color, bool selectable, bool subSelectable, bool selected, uint32_t id, uint32_t subIDOffset) {
	...
	GLCall(glBufferSubData(GL_UNIFORM_BUFFER, sizeof(glm::mat4) + sizeof(glm::vec3) + sizeof(uint32_t) + sizeof(uint32_t), sizeof(uint32_t), &subIDOffset));
}
